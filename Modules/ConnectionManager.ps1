...
function Start-Connection {
    <#
    .SYNOPSIS
    # Ensure a fresh cancellation token for each start. Reusing a cancelled
    # CancellationTokenSource keeps the token in a cancelled state and any
    # subsequent worker thread will exit immediately. Re-create it before
    # launching the protocol-specific worker.
    if (-not $conn.CancellationSource -or $conn.CancellationSource.IsCancellationRequested) {
        try {
            if ($conn.CancellationSource) {
                $conn.CancellationSource.Dispose()
            }
        } catch {
            # Ignore disposal issues - we only want a fresh token for the next run.
        }

        $conn.CancellationSource = New-Object System.Threading.CancellationTokenSource
    }
...
function Stop-Connection {
    <#
    .SYNOPSIS
        if ($conn.CancellationSource -and -not $conn.CancellationSource.IsCancellationRequested) {
    #>
    param(
        [Parameter(Mandatory=$true)]
        [string]$ConnectionId,
