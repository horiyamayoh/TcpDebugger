...
## アーキテクチャ概要
- `TcpDebugger.ps1` で Modules/ 以下の通信・自動応答モジュールと UI/MainForm.ps1 を dot-source し、WinForms 上で接続操作・シナリオ実行・受信ルール設定を一元化している。
- `ConnectionManager.ps1` の ConnectionContext が送信キュー/受信バッファ/変数スコープ/タイマーを同期付きで保持し、`TcpClient.ps1` `TcpServer.ps1` `UdpCommunication.ps1` の各スレッドがこれを消費する。
- `ScenarioEngine.ps1` は CSV ベースの SEND/WAIT/LOOP/TIMER アクションを解釈し、`QuickSender.ps1` は `templates/databank.csv` を読み出して定型メッセージを生成する。
- `AutoResponse.ps1` + `ReceivedRuleEngine.ps1` が AutoResponse/OnReceived/Unified ルールを判別してテンプレートやスクリプトを展開し、`MessageHandler.ps1`/`OnReceivedLibrary.ps1` がテンプレート展開やバイト操作 API を提供する。
- `UI/MainForm.ps1` の DataGridView は Auto Response / On Received / Periodic Send / Quick Action 列を通じて `Set-ConnectionAutoResponseProfile` などの API を直接呼び、プロファイル切替やシナリオ実行を即時反映できる。
...
## 既知の制限
- `TcpClient.ps1` と `UdpCommunication.ps1` では `Invoke-ConnectionAutoResponse` の呼び出し位置が受信処理より前にあり、`receivedData` が未定義のまま評価される恐れがある。
- 受信パイプラインは `ReceivedEventHandler.ps1` を経由する想定だが、通信ループから `Invoke-ReceivedEvent` が呼ばれておらず、OnReceived プロファイル単体では発火しない。
- `UI/MainForm.ps1` の Periodic Send 設定で未定義の `Get-InstancePath` を参照しており、`Connection.Variables['InstancePath']` などを使う修正が必要。
- `ScenarioEngine.ps1` の IF アクション (`Invoke-IfAction`) は未実装の警告だけを出すため、条件分岐付きシナリオはまだ実行できない。
- OnReceived プロファイル切替は Unified ルール経由でのみ効果があり、純粋な OnReceived CSV を生かすには受信ループへ `Invoke-ConnectionOnReceived` を組み込む必要がある。
