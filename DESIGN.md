...
### 4.2 接続設定ファイル（connections.csv） ※旧形式・後方互換用
この形式は `Config/connections.csv` に配置し、一括インポート時や簡易設定に使用できるが、推奨はインスタンスフォルダ方式。
```csv
Name,Protocol,Mode,LocalIP,LocalPort,RemoteIP,RemotePort,AutoStart,ScenarioFile
Conn1,TCP,Client,0.0.0.0,0,192.168.1.100,8080,true,scenario1.csv
Conn2,TCP,Server,0.0.0.0,9000,,,true,
Conn3,UDP,Sender,0.0.0.0,0,192.168.1.200,5000,false,
```
